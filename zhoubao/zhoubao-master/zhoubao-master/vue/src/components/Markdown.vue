<template>
	<div id="editor" v-loading="loading">
		<div class="boxFinding">
			<el-button type="primary" v-if="buttonPrimary" @click="confirmSaveMarkdown">保存</el-button>
			<el-button type="success" v-if="!buttonPrimary">已上传</el-button>

			<el-button @click="input=''">编写</el-button>
			<el-button @click="getMdStr('../static/周报模板.md')">使用模板编写</el-button>
		</div>
		<br />
		<el-row :gutter="75" type="flex" justify="left">
			<el-col :span="10">
	...
	export default {
		...
		methods: {
			...
			confirmSaveMarkdown: function() {
				this.$confirm('此操作会上传Markdown, 是否继续?', '提示', {
